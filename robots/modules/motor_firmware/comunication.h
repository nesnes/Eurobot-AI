...
#endif
#ifdef MODE_SERIAL
  #define COMUNICATION_BUFFER_IN_SIZE 320
  #define COMUNICATION_BUFFER_OUT_SIZE 320
#endif
...
/* Serial communication */
#ifdef MODE_SERIAL  
  void readSerial() {
    while (Serial.available() && !comunication_ReceiveFlag) {
      char inChar = (char)Serial.read();
      if (inChar == '\r' || inChar == '\n' || serialReadIndex>=COMUNICATION_BUFFER_IN_SIZE-2) {
        comunication_ReceiveFlag = serialReadIndex > 0;
        comunication_InBuffer[serialReadIndex+1] = '\0';
        if(comunication_ReceiveFlag) serialReadIndex = 0;
        break;
      }
      else{
        comunication_InBuffer[serialReadIndex++] = inChar;
      }
    }
  }
  ...
  void sendSerial(){
    if(comunication_SendFlag){
      comunication_OutBuffer[COMUNICATION_BUFFER_OUT_SIZE-1] = '\0';
